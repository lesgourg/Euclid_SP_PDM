...
def initiate (self, data):
    ...
    self.need_cosmo_arguments(data, {'P_k_max_h/Mpc': self.kmax_in_h_by_Mpc})
    self.need_cosmo_arguments(data, {'non linear': 'halofit'} if self.use_halofit else {})
...
def read_data (self):
    ...
    if self.probe_WL and not self.probe_GC:
        fid_file_path = os.path.join(self.data_directory, self.fiducial_file_WL)
    elif self.probe_GC and not self.probe_WL:
        fid_file_path = os.path.join(self.data_directory, self.fiducial_file_GC)
    else:
        fid_file_path = os.path.join(self.data_directory, self.fiducial_file_XC)
    if os.path.exists(fid_file_path):
        fid_file = open(fid_file_path, 'r')
        ...
        if self.probe_WL and not self.probe_GC:
            self.Cov_observ = np.zeros((self.lmax_WL-self.lmin+1, self.nbin, self.nbin), 'float64')
            for Bin1 in range(self.nbin):
                for Bin2 in range(self.nbin):
                    for nl in range(self.lmax_WL-self.lmin+1):
                        self.Cov_observ[nl,Bin1,Bin2] = float(line)
                        line = fid_file.readline()

        elif self.probe_GC and not self.probe_WL:
            self.Cov_observ = np.zeros((self.lmax_GC-self.lmin+1, self.nbin, self.nbin), 'float64')
            for Bin1 in range(self.nbin):
                for Bin2 in range(self.nbin):
                    for nl in range(self.lmax_GC-self.lmin+1):
                        self.Cov_observ[nl,Bin1,Bin2] = float(line)
                        line = fid_file.readline()

        else:
            self.Cov_observ = np.zeros((min(self.lmax_GC, self.lmax_WL)-self.lmin+1, 2*self.nbin, 2*self.nbin), 'float64')
            self.Cov_observ_high = np.zeros((max(self.lmax_WL, self.lmax_GC) - min(self.lmax_WL, self.lmax_GC), self.nbin, self.nbin), 'float64')
            for Bin1 in range(2*self.nbin):
                for Bin2 in range(2*self.nbin):
                    for nl in range(min(self.lmax_GC, self.lmax_WL)-self.lmin+1):
                        self.Cov_observ[nl,Bin1,Bin2] = float(line)
                        line = fid_file.readline()
            for Bin1 in range(self.nbin):
                for Bin2 in range(self.nbin):
                    for nl in range(max(self.lmax_WL, self.lmax_GC) - min(self.lmax_WL, self.lmax_GC)):
...
def window_functions_WL(self, cosmo, data, k, kmin, kmax):
    ...
    if not self.scale_dependent_f:
        ...
        for index_z, z in enumerate(self.z):
            D_z[index_z] = cosmo.scale_independent_growth_factor(z)
        ...
        for index_z in range(self.nzmax):
            ls_to_probe = np.where((k[:,index_z]>kmin) & (k[:,index_z]<kmax))
            D_z[ls_to_probe,index_z] = np.sqrt(Pk_l(self.z[index_z], k[ls_to_probe,index_z]) / Pk_l(0, k[ls_to_probe,index_z]))
