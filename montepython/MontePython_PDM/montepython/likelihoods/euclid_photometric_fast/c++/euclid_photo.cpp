            ...
            for (int k = j; k < nbin; k++) {
                for (int nz = 0; nz < nzmax; nz++) {
                    if (scale_dependent_f_low)
                        Cl_int[nz] = 1.* W_G[i*nzmax*nbin + nz*nbin + j] * W_G[i*nzmax*nbin + nz*nbin + k] * pk[i*nzmax + nz] / H_z[nz] / r[nz] / r[nz];
                    else
                        Cl_int[nz] = 1.* W_G[nz*nbin + j] * W_G[nz*nbin + k] * pk[i*nzmax + nz] / H_z[nz] / r[nz] / r[nz];
                }
                double integrated_Cl = 1.* trapz(Cl_int, z, nzmax);
    ...
    for (int i = 0; i < lbin_low; i++) {
        for (int j = 0; j < nbin; j++) {
            for (int k = 0; k < nbin; k++) {
                for (int nz = 0; nz < nzmax; nz++) {
                    if (scale_dependent_f_high)
                        Cl_int[nz] = 1.* W_L[i*nzmax*nbin + nz*nbin + j] * W_G[nz*nbin + k] * pk[i*nzmax + nz] / H_z[nz] / r[nz] / r[nz];
                    else if (scale_dependent_f_low)
                        Cl_int[nz] = 1.* W_L[nz*nbin + j] * W_G[i*nzmax*nbin + nz*nbin + k] * pk[i*nzmax + nz] / H_z[nz] / r[nz] / r[nz];
                    else
                        Cl_int[nz] = 1.* W_L[nz*nbin + j] * W_G[nz*nbin + k] * pk[i*nzmax + nz] / H_z[nz] / r[nz] / r[nz];
                }
                double integrated_Cl = 1.* trapz(Cl_int, z, nzmax);
