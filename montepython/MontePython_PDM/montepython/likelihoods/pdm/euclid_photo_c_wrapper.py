...
try:
    # _photo = CDLL(os.path.dirname(os.path.realpath(__file__)) + '/../euclid_photometric_fast/c++/euclid_photo.so')
    _photo = CDLL(os.path.dirname(os.path.realpath(__file__)) + '/c++/euclid_photo.so')
except:
    print("Compiling shared library %s/c++/euclid_photo.so..." % os.path.dirname(os.path.realpath(__file__)))
...
def chi2_xc (self, W_L, W_G, pk, pk_no_bf, pk_sqrt_bf):
    if self.print_individual_times:
        self.chunk_time = time()
    chi2 = _photo.chi2_xc (
        self.nzmax,
