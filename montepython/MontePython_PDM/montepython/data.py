...
class Data(object):
    ...
    def update_cosmo_arguments(self):
        ...
        self.lcdm_pars = {
            ...
            'n_s'               : self.cosmo_arguments['n_s'],
            'ln10^{10}A_s'      : self.cosmo_arguments['ln10^{10}A_s'],
            'Omega_m'           : self.cosmo_arguments['Omega_m']
        }

        if 'm_nu' in self.cosmo_arguments:
            self.m_nu = self.cosmo_arguments['m_nu'].copy()

        if self.has_cwdm:
            f_wdm = 10**self.cosmo_arguments['log10f_wdm']
            del self.cosmo_arguments['log10f_wdm']
            ...
            h1 = self.cosmo_arguments['h']
            Omega_dm = self.cosmo_arguments['Omega_m'] - self.cosmo_arguments['Omega_b']
            omega_dm = Omega_dm * h1**2

            x = (m_wdm/(93.14*omega_dm * f_wdm)**0.25)**(4./3.)
            Twdm_over_Tgamma = (omega_dm * f_wdm*93.14)**0.25 \
                * x**-0.25 * (4./11.)**(1./3.)

            self.cosmo_arguments['omega_ncdm'] = omega_dm * f_wdm
            self.cosmo_arguments['omega_cdm'] = omega_dm * (1-f_wdm)
            self.cosmo_arguments['T_ncdm'] = Twdm_over_Tgamma
            self.cosmo_arguments['m_ncdm'] = m_wdm

            del self.cosmo_arguments['Omega_m']
            skip_Omega_m = True
        
        if self.has_onebody:
            f_ddm_ini   = self.cosmo_arguments['f_ddm_ini']
            Gamma       = self.cosmo_arguments['Gamma_times_f'] / f_ddm_ini
            del self.cosmo_arguments['Gamma_times_f']
            del self.cosmo_arguments['f_ddm_ini']
            ...
            Omega_dm = self.cosmo_arguments['Omega_m'] - self.cosmo_arguments['Omega_b']
            self.cosmo_arguments['Omega_cdm'] = Omega_dm * (1-f_ddm_ini)
            self.cosmo_arguments['Omega_ini_dcdm'] = Omega_dm * f_ddm_ini
            self.cosmo_arguments['Gamma_dcdm'] = Gamma * 977.792
        ...
        if self.has_twobody:
            try:
                f_ddm_ini   = 10 ** self.cosmo_arguments['logf']
                Gamma       = 10 ** self.cosmo_arguments['log\Gamma']
                epsilon     = 10 ** self.cosmo_arguments['log\epsilon']
                del self.cosmo_arguments['logf']
                del self.cosmo_arguments['log\Gamma']
                del self.cosmo_arguments['log\epsilon']
            except:
                Gamma       = self.cosmo_arguments['Gamma_2b_dcdm_inv_Gyr']
                f_ddm_ini   = self.cosmo_arguments['f_2b_ddm_0']
                epsilon     = self.cosmo_arguments['epsilon']
                del self.cosmo_arguments['Gamma_2b_dcdm_inv_Gyr']
                del self.cosmo_arguments['f_2b_ddm_0']
                del self.cosmo_arguments['epsilon']

            self.twobody_pars = {
                'Gamma'         : Gamma,
                'f_2b_ddm_0'    : f_ddm_ini,
                'velocity_kick' : 299792.458 * epsilon/(1-epsilon)
            }

            Omega_dm = self.cosmo_arguments['Omega_m'] - self.cosmo_arguments['Omega_b']
            self.cosmo_arguments['Omega_ini_dcdm2'] = Omega_dm * f_ddm_ini
            self.cosmo_arguments['Omega_cdm'] = Omega_dm * (1-f_ddm_ini) + 1e-5
            self.cosmo_arguments['Gamma_dcdm'] = Gamma * 977.792
            self.cosmo_arguments['epsilon_dcdm'] = epsilon

            del self.cosmo_arguments['Omega_m']
            skip_Omega_m = True
        
        if self.has_ethos:
            xi_idr = 10 ** self.cosmo_arguments['log10xi_idr']
            self.ethos_pars = {
                'log10a_dark'   : self.cosmo_arguments['log10a_dark'],
                'xi_idr'        : xi_idr
            }
            self.cosmo_arguments['a_idm_dr'] = 10 ** self.cosmo_arguments['log10a_dark']
            self.cosmo_arguments['xi_idr'] = xi_idr
            del self.cosmo_arguments['log10a_dark']
            del self.cosmo_arguments['log10xi_idr']

        ################################
        # END EUCLID PDM MODIFICATIONS #
        ################################
        ...
        for elem in self.get_mcmc_parameters(['cosmo']):
            ...
            elif elem == 'Omega_m' and not skip_Omega_m:
            # ^ Euclid PDM modification
                Omega_m = self.cosmo_arguments['Omega_m']
                if "N_ncdm" in self.cosmo_arguments and self.cosmo_arguments["N_ncdm"]>0:
                    omega_nu = 0.
                    if "m_ncdm" in self.cosmo_arguments:
